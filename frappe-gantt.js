...
class Bar {
    ...
    draw_avatar(){
      ...
      requestAnimationFrame(() => this.update_avatar_position());
    }
    draw_progress_bar() {
        if (this.invalid) return;
    ...
    update_avatar_position() {
        const bar = this.$bar,
            avatar = this.group.querySelector('.bar-avatar');
        avatar.setAttribute('x', bar.getX()-bar.getHeight());
    }
...
class Gantt {
    ...
    refresh(tasks) {
      this.setup_wrapper(".gantt-target");
        this.setup_tasks(tasks);
        this.change_view_mode();
        this.bind_events();
    }
